...
function App() {
  ...
  const onlineServerPct = Math.round((data.serversfeat: add HTTP health checks and improve dashboard developmentOnline / data.serversTotal) * 100);
  ...
  return <div className="app-shell">
    ...
    <main className="main"><header><button className="menu-button" onClick={() => setMenuOpen(true)}><Menu size={21}/></button><div><p className="eyebrow">{today}</p><h1>Good evening, Alex<span className="accent">.</span></h1></div><div className="header-actions"><button className="icon-button refresh-button" onClick={refresh} title="Refresh dashboard"><RefreshCw size={17} className={loading ? 'spin' : ''}/></button><button className="icon-button"><Bell size={19}/><i></i></button><div className="avatar">A</div></div></header>
      ...
      <section className="panel services-panel"><PanelTitle title="Services" action="Manage services"/><div className="services-grid">{data.services.map(service => <div className="service-card" key={service.id}><div className="service-top"><div className="service-logo">{service.name.slice(0,1)}</div><span className={`service-status ${service.status === 'Online' ? '' : 'down'}`}><span></span>{service.status}</span></div><strong>{service.name}</strong><span className="response">{service.status === 'Online' ? `${service.responseTime}ms response time` : 'Check required'}</span><small className="last-check">{service.lastCheck ? `Checked ${formatRelativeTime(service.lastCheck)}` : 'Waiting for first check'}</small></div>)}</div></section>
